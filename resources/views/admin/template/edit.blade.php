              ...
              <form class="needs-validation" novalidate action="{{ route($route.'.update', $row->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="title">{{ __('dashboard.subject') }} <span>*</span></label>
                    ...
                    <div class="form-group">
                        <label for="description">{{ __('dashboard.message') }} <span>*</span></label>
                        <textarea class="form-control summernote" name="description" id="description" rows="8">{!! $row->description !!}</textarea>
                        @if($row->slug != 'subscription')
                        <div class="alert alert-secondary" role="alert">
                            {{ __('dashboard.shortcode') }}: 
                            [name] [email] [phone] [address] [city] [company] [services]
                        </div>
                        @endif
